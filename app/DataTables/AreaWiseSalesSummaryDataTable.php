...
class AreaWiseSalesSummaryDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        return (new EloquentDataTable($query))
            ...
            ->addColumn('total_sum_amount', function ($row) use ($year, $month) {
                ...
                return $query->whereNotNull('area_id')->count();
            })
            ->addColumn('total_count', function ($row) use ($year, $month) {
                $start_date = $year . '-' . $month . '-01';
                $end_date = date('Y-m-t', strtotime($year . '-' . $month));
                $count = Order::where('area_id', $row->id)->where('date', '>=', $start_date)->where('date', '<=', $end_date)->whereIn('status', ['Delivered', 'Collected'])->count();
                if ($count > 0) {
                    return '<a href="' . Route('admin.area-wise-sales-statement.index') . '?view_orders=true&start_date=' . $start_date . '&end_date=' . $end_date . '&area_id=' . $row->id . '">' . $count . '</a>';
                } else {
                    return '-';
                }
            })
            ...
            ->addColumn('one', function ($row) use ($month, $year) {
                $date = $year . '-' . $month . '-01';
                $count =  Order::where('area_id', $row->id)->where('date', $date)->whereIn('status', ['Delivered', 'Collected'])->count();
                if ($count > 0) {
                    return '<a href="' . Route('admin.area-wise-sales-statement.index') . '?view_orders=true&date=' . $date . '&area_id=' . $row->id . '">' . $count . '</a>';
                } else {
                    return '-';
                }
            })
    ...
    {
        return 'area_wise_sales_summary' . date('d_m_Y_h_i_s_A');
    }
}
