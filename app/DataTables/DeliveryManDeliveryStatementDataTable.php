...
class DeliveryManDeliveryStatementDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        return (new EloquentDataTable($query))
            ...
            ->addColumn('total_sum_amount', function ($row) use ($year, $month) {
                ...
                $query = Order::where('date', '>=', $start_date)->where('date', '<=', $end_date)->whereIn('status', ['Delivered', 'Collected']);
                if (request('store_id')) {
                    $query->where('store_id', request('store_id'));
                }
                if (request('delivery_man_id')) {
                    $query->whereIn('delivery_man_id', request('delivery_man_id'));
                }
                return $query->count();
            })
            ->addColumn('total_count', function ($row) use ($year, $month) {
                $start_date = $year . '-' . $month . '-01';
                $end_date = date('Y-m-t', strtotime($year . '-' . $month));
                $count = Order::where('delivery_man_id', $row->id)->where('date', '>=', $start_date)->where('date', '<=', $end_date)->whereIn('status', ['Delivered', 'Collected'])->count();
                if ($count > 0) {
                    return '<a href="' . Route('admin.deliveryman-delivery-statement.index') . '?view_orders=true&start_date=' . $start_date . '&end_date=' . $end_date . '&delivery_man_id=' . $row->id . '">' . $count . '</a>';
                } else {
                    return '-';
                }
            })
            ...
            ->addColumn('one', function ($row) use ($month, $year) {
                $date = date('Y-m-d', strtotime($year . '-' . $month . '-01'));
                $count =  Order::where('delivery_man_id', $row->id)->where('date', $date)->whereIn('status', ['Delivered', 'Collected'])->get();
                $count = $count->count();
                if ($count > 0) {
                    return '<a href="' . Route('admin.deliveryman-delivery-statement.index') . '?view_orders=true&date=' . $date . '&delivery_man_id=' . $row->id . '">' . $count . '</a>';
                } else {
                    return '-';
                }
            })
            ...
            ->addColumn('two', function ($row) use ($month, $year) {
                $date = date('Y-m-d', strtotime($year . '-' . $month . '-02'));
                $count =  Order::where('delivery_man_id', $row->id)->where('date', $date)->whereIn('status', ['Delivered', 'Collected'])->count();
                if ($count > 0) {
                    return '<a href="' . Route('admin.deliveryman-delivery-statement.index') . '?view_orders=true&date=' . $date . '&delivery_man_id=' . $row->id . '">' . $count . '</a>';
                } else {
                    return '-';
            ...
            ->addColumn('total_twenty_nine', function ($row) use ($month, $year) {
                ...
                if ($date == $year . '-' . $month . '-29') {
                    $count = Order::where('date', $date)->whereIn('status', ['Delivered', 'Collected'])->count();
                    return $count > 0 ? $count : '-';
                } else {
                    return '-';
                }
    ...
    {
        return 'delivery_man_delivery_statement_' . date('d_m_Y_h_i_s_A');
    }
}
