...
class HelperClass
{
    ...
    public static function stock($product_id, $product_type = 'Consumer')
    {
        ...
        if ($product_type == 'Fashion') {
            ...
            $lifting_returns = DB::table('view_lifting_returns')->where('product_type', $product_type)->where('sku_id', $product_id)->sum('qty');
            $sales = DB::table('view_sales')->where('product_type', $product_type)->where('sku_id', $product_id)->sum('qty');
