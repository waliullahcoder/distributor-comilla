...
class ActionButtons
{
    public static function actions($data, $addiotional_buttons = NULL, $delete_check = 'yes', $permit_edit = 'yes')
    {
        ...
        if (is_array($addiotional_buttons) &&  count($addiotional_buttons) > 0) {
            foreach ($addiotional_buttons as $item) {
                ...
                if (!is_null($currentRoutePermission) && Auth::user()->can(@$currentRoutePermission->name)) {
                    $actions .= '<a href="' . ($item['parameter'] ? Route($item['route'], $data['id']) : Route($item['route'])) . '" class="' . @$item['class'] . ' tt" target="' . @$item['target'] . '" data-id="' . $data['id'] . '" data-bs-toggle="tooltip" data-bs-placement="top" title="' . @$item['title'] . '">' . @$item['icon'] . @$item['text'] . '</a>';
                }
            }
        } elseif (isset($addiotional_buttons['route'])) {
            $currentRoutePermission = Permission::where('name', $addiotional_buttons['route'])->first();
            if (!is_null($currentRoutePermission) && Auth::user()->can(@$currentRoutePermission->name)) {
                $actions .= '<a href="' . ($addiotional_buttons['parameter'] ? Route($addiotional_buttons['route'], $data['id']) : Route($addiotional_buttons['route'])) . '" class="' . @$addiotional_buttons['class'] . ' tt" target="' . @$addiotional_buttons['target'] . '" data-id="' . $data['id'] . '" data-bs-toggle="tooltip" data-bs-placement="top" title="' . @$addiotional_buttons['title'] . '">' . @$addiotional_buttons['icon'] . @$addiotional_buttons['text'] . '</a>';
            }
