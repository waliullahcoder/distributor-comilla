...
class OnlineOrderController extends Controller
{
        ...
        if (request()->ajax()) {
            ...
            return DataTables::eloquent($model)
                ...
                ->addColumn('actions', function ($row) {
                    ...
                    if (in_array($row->status, ['Pending', 'Forward', 'On Route']) && is_null($coll_transaction)) {
    ...
    public function create(Request $request)
    {
        if ($request->ajax() && $request->has('user_phone')) {
            $order = Order::where('user_phone', 'like', $request->user_phone)->orderBy('id', 'desc')->first();
            if ($order) {
                return response()->json(['status' => 'success', 'user_name' => $order->user_name, 'shipping_address' => $order->shipping_address]);
            }
        }

        if ($request->ajax() && $request->has('get_package')) {
            $package = OrderPackage::findOrFail($request->order_package_id);
            return response()->json([
                'status' => 'success',
                'data' => view('admin.online_order.partial.table', compact('package'))->render()
            ]);
        }
        ...
        return view('admin.online_order.create', compact('title', 'order_no', 'products', 'areas', 'packages'));
    }
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($request) {
            ...
            $data = Order::create([
                ...
                'user_phone_alt' => $request->user_phone_alt,
                ...
                'paid' => 0,
                'due' => round($request->total_amount + $request->shipping_charge - $request->discount),
            ...
            foreach ($request->product_id as $key => $product_id) {
                $discount = $request->total_amount > 0 ? ($request->discount / $request->total_amount) * $request->amount[$key] : 0;
                OrderProduct::create([
                    'order_id' => $data->id,
    ...
    public function update(Request $request, string $id)
    {
        ...
        DB::transaction(function () use ($request, $id) {
            ...
            $data->update([
                ...
                'advance' => $request->advance ?? $data->advance,
                'date' => date('Y-m-d', strtotime($request->date)),
                'potential_delivery_date' => date('Y-m-d', strtotime($request->potential_delivery_date)),
                'order_note' => $request->order_note,
                // 'created_by' => $request->created_by ?? $data->created_by,
            ]);
            ...
            $transactions = AccountTransactionAuto::withTrashed()->where('voucher_no', $data->invoice)->where('voucher_type', 'Retail Sales')->count();
            if($transactions > 0){
