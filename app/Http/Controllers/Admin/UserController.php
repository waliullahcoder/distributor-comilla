        ...
        if (request()->ajax()) {
            ...
            return DataTables::eloquent($model)
                ...
                ->addColumn('actions', function ($row) {
                    ...
                    $type = request('type');
                    if (!empty($type) && $type == 'trash') {
                        $actionBtn .= '<button type="button" class="btn btn-sm border-0 px-10px tt btn-success link-recovery" data-url="' . Route('admin.user.destroy', $row->id) . '" data-bs-toggle="tooltip" data-bs-placement="top" title="Recovery"><i class="fad fa-recycle"></i></button>';
                    } else {
                        if ($row->role == 1) {
                            if (!in_array('Software Admin', json_decode($row->getRoleNames())) && Auth::user()->hasRole('Software Admin') || Auth::user()->can('admin.user.edit') && !in_array('Software Admin', json_decode($row->getRoleNames()))) {
                                $actionBtn .= '<a href="' . Route('admin.user.edit', $row->id) . '" class="btn btn-sm btn-warning border-0 px-10px tt link-edit" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit"><i class="far fa-pencil-alt"></i></a>';
                            }
                        }
                        if (!in_array('Software Admin', json_decode($row->getRoleNames())) && Auth::user()->can('admin.user.password')) {
                            $actionBtn .= '<a href="' . Route('admin.user.password', $row->id) . '" class="btn btn-sm btn-warning border-0 px-10px tt bg-success" data-bs-toggle="tooltip" data-bs-placement="top" title="Change Password"><i class="fal fa-key"></i></a>';
                        }
                    }
                    if ($row->role == 1) {
                        if (!in_array('Software Admin', json_decode($row->getRoleNames())) && Auth::user()->hasRole('Software Admin') || Auth::user()->can('admin.user.destroy') && !in_array('Software Admin', json_decode($row->getRoleNames()))) {
                            $actionBtn .= '<button type="button" class="btn btn-sm btn-danger border-0 px-10px tt ' . (!empty($type) && $type == 'trash' ? 'trash_delete' : 'link-delete') . '" data-url="' . Route('admin.user.destroy', $row->id) . '" data-bs-toggle="tooltip" data-bs-placement="top" title="Delete"><i class="far fa-trash-alt"></i></button>';
                        }
                    }
                    $actionBtn .= '</div>';
                    return $actionBtn;
