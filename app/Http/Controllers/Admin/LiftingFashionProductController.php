...
class LiftingFashionProductController extends Controller
{
    ...
    public function invoice()
    {
        ...
        $data = Lifting::withoutGlobalScope(CompanyScope::class)->withTrashed()->select(['lifting_no'])->where('created_at', '>=', $first)->where('created_at', '<=', $last)->latest('id')->first();
        if ($data) {
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($request, $admin_setting) {
            ...
            if ($request->payment_type == 'cash') {
                $first = date('Y-m-01');
                $last = new Carbon('last day of this month');
                $pay_data = VendorPayment::withoutGlobalScope(CompanyScope::class)->withTrashed()->select(['payment_no'])->where('created_at', '>=', $first)->where('created_at', '<=', $last)->latest('id')->first();
                if ($pay_data) {
