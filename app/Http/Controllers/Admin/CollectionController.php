...
class CollectionController extends Controller
{
    ...
    public function index()
    {
        if (request()->ajax()) {
            ...
            return DataTables::eloquent($model)
                ->addColumn('checkbox', function ($row) {
                    $transaction = AccountTransaction::withTrashed()->where('voucher_no', $row->payment_no)->where('voucher_type', 'Collection')->first();
        ...
        $params = '<input type="text" class="form-control date px-2 py-1" id="date" name="date" style="width: 150px; min-height: auto;" placeholder="Collection Date">';
    ...
    public function invoice()
    {
        ...
        $data = Collection::withoutGlobalScope(CompanyScope::class)->withTrashed()->select(['payment_no'])->where('created_at', '>=', $first)->where('created_at', '<=', $last)->latest('id')->first();
        if ($data) {
    ...
    public function create(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $balance = round(($total_paid_amount + $returnAmount) - $total_sale_amount, 2);
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($request, $amount) {
            ...
            if ($client->coa && $request->coa_setup_id) {
