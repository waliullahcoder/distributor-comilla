...
class OfflineOrderController extends Controller
{
    ...
    public function invoice()
    {
        ...
        $order = Order::withoutGlobalScope(CompanyScope::class)->withTrashed()->select(['invoice'])->where('created_at', '>=', $first)->where('created_at', '<=', $last)->where('order_type', 'offline')->latest('id')->first();
        if ($order) {
