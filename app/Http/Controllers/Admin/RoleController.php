...
class RoleController extends Controller
{
    ...
    {
        if (request()->ajax()) {
            $model = Role::with('users')->whereNotIn('name', ['Software Admin'])->orderBy('id', 'desc');
            $company_id = Auth::user()->company_id;
            if ($company_id) {
                $model->where('company_id', $company_id);
            }
            if (!Auth::user()->hasRole('Software Admin')) {
                $model->whereNotIn('name', ['System Admin', 'Client', 'Investor', 'Moderator']);
            }
            return DataTables::eloquent($model)
                ...
                ->addColumn('actions', function ($row) {
                    ...
                    if (count($row->users) > 0) {
                        return ActionButtons::actions($data, $addiotional_buttons, 'no', 'yes');
                    }
