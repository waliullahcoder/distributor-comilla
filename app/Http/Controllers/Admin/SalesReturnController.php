...
class SalesReturnController extends Controller
{
    ...
    public function invoice()
    {
        ...
        $data = SalesReturn::withoutGlobalScope(CompanyScope::class)->withTrashed()->select(['return_no'])->where('created_at', '>=', $first)->where('created_at', '<=', $last)->latest('id')->first();
        if ($data) {
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($request) {
            ...
            foreach ($request->sales_list_id as $sales_list_id) {
                ...
                $total_return_amount = $sales_list->returned_amount + ($sales_list->rate * $request->return_qty[$sales_list_id]);
