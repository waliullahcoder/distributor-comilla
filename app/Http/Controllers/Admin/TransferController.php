...
class TransferController extends Controller
{
        ...
        if (request()->ajax()) {
            ...
            return DataTables::eloquent($model)
                ...
                ->addColumn('actions', function ($row) {
                    ...
                    if ($row->approve == 0 && $row->reject == 0) {
                        if (!empty($type) && $type == 'trash') {
                            $actionBtn .= '<button type="button" class="btn btn-sm border-0 px-10px tt btn-success link-recovery" data-url="' . Route('admin.transfer.destroy', $row->id) . '" data-bs-toggle="tooltip" data-bs-placement="top" title="Recovery"><i class="fad fa-recycle"></i></button>';
                        } else {
                            $actionBtn .= '<a href="' . Route('admin.transfer.edit', $row->id) . '" class="btn btn-sm btn-warning border-0 px-10px tt link-edit" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit"><i class="far fa-pencil-alt"></i></a>';
                        }
                    }
                    $actionBtn .= '<a class="btn btn-sm border-0 px-10px tt btn-info" href="' . Route('admin.transfer.show', $row->id) . '" target="_blank" data-bs-toggle="tooltip" data-bs-placement="top" title="Print Voucher"><i class="fal fa-print"></i></a>';
                    if ($row->approve == 0 && $row->reject == 0) {
                        $actionBtn .= '<button type="button" class="btn btn-sm border-0 px-10px tt btn-danger ' . (!empty($type) && $type == 'trash' ? 'trash_delete' : 'link-delete') . '" data-url="' . Route('admin.transfer.destroy', $row->id) . '" data-bs-toggle="tooltip" data-bs-placement="top" title="Delete"><i class="far fa-trash-alt"></i></button>';
                    }
                    $actionBtn .= '</div>';
                    return $actionBtn;
                })
    ...
    public function invoice()
    {
        ...
        $data = Transfer::withoutGlobalScope(CompanyScope::class)->withTrashed()->select(['transfer_no'])->where('created_at', '>=', $first)->where('created_at', '<=', $last)->latest('id')->first();
        if ($data) {
