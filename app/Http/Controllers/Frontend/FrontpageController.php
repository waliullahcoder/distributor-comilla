...
use App\Models\{Page, Slider, Product, Category, About, Campaign, Contact, Order, OrderPackage, OrderProduct, PreOrderSetup, SocialWork, StaticSiteItem};
...
class FrontpageController extends Controller
{
    ...
    public function invoice()
    {
        ...
        $order = Order::withTrashed()->select(['invoice'])->where('created_at', '>=', $first)->where('created_at', '<=', $last)->orderBy('id', 'desc')->first();
        if ($order) {
    ...
    {
        if ($request->ajax()) {
            $data = OrderPackage::findOrFail($request->order_package_id);
            $subtotal = ($data->amount - $data->discount) * $request->qty;
            $net_amount = $subtotal + $data->shipping_charge;
            return response()->json(['status' => 'success', 'data' => $data, 'subtotal' => $subtotal, 'net_amount' => $net_amount]);
        }

        $data = Campaign::with(['packages', 'list', 'reviews'])->where('slug', $slug)->where('status', 1)->first();
        return view('frontend.campaign', compact('data'));
    }

    public function packageCheckout(Request $request, string $slug)
    {
        $data = OrderPackage::where('slug', $slug)->first();
        return view('frontend.campaign_checkout', compact('data'));
    }
    ...
    public function packageOrder(Request $request)
    {
        ...
        DB::transaction(function () use ($request, &$data) {
            ...
            $delivery_date = new DateTime();
            $delivery_date->modify('+72 hours');
            $order_package = OrderPackage::findOrFail($request->order_package_id);
            $qty = $request->qty;
            $subtotal = $order_package->amount * $qty;
            $discount = $order_package->discount > 0 ? $order_package->discount * $qty : 0;

            $data = Order::create([
                'company_id' => 1,
                'order_package_id' => $request->order_package_id,
                'invoice' => $invoice,
                'order_code' => 'R' . mt_rand(111111, 999999),
                'user_name' => $request->user_name,
                'user_phone' => $request->user_phone,
                'shipping_address' => $request->shipping_address,
                'shipping_charge' => $order_package->shipping_charge,
                'sub_total' => $subtotal + $discount,
                'total' => $subtotal + $discount + $order_package->shipping_charge,
                'discount' => $discount,
                'paid' => 0,
                'due' => $subtotal + $order_package->shipping_charge,
                'order_type' => 'online',
                'status' => 'Pending',
                'coupon_id' => NULL,
                'order_note' => NULL,
                'payment_method' => 'COD',
                'order_note' => $request->remarks,
                'date' => date('Y-m-d'),
                'potential_delivery_date' => $delivery_date,
                'pending_at' => Carbon::now(),
                'created_by' => 0,
            ]);

            foreach ($order_package->list as $item) {
                if ($order_package->discount > 0) {
                    $discount = ($order_package->discount / $order_package->amount) * $item->amount;
        ...
        return redirect()->route('frontend.thank-you', $data->id);
    }

    public function thankYou(Request $request, $id)
    {
        $data = Order::find($id);
        return view('frontend.checkout_thankyou', compact('data'));
    }
}
