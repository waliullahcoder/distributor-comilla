    ...
    <div class="row g-3">
        ...
        <div class="col-lg-3 col-sm-6">
            ...
            <input type="text" class="form-control date_picker" id="potential_delivery_date"
                ...
                value="{{ old('potential_delivery_date') ? date('d-m-Y', strtotime(old('potential_delivery_date'))) : $delivery_date->format('d-m-Y') }}"
                ...
                <select name="created_by" id="created_by" class="select form-select" data-placeholder="Select Staff"
                    ...
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}"
                            {{ old('created_by') && old('created_by') == $user->id ? 'selected' : '' }}>{{ $user->name }}
                        </option>
                    @endforeach
                </select>
            </div>
        @else
            <div class="col-lg-3 col-sm-6">
                ...
                <input type="text" class="form-control" id="created_by" value="{{ Auth::user()->name }}"
        ...
        <div class="col-md-3 col-sm-6">
            ...
            <div class="input-group">
                <input type="number" class="form-control" id="user_phone" name="user_phone" placeholder="Customer Phone"
                    required>
                <button class="btn border rounded-0" id="alt_phone" type="button"><i class="far fa-plus"></i></button>
            </div>
            <input type="number" class="form-control mt-1" id="user_phone_alt" name="user_phone_alt"
                placeholder="Customer Phone" style="display: none;">
        </div>
        <div class="col-md-3 col-sm-6">
            <label for="user_name" class="form-label"><b>Customer Name <span class="text-danger">*</span></b></label>
            <input type="text" class="form-control" id="user_name" name="user_name" placeholder="Customer Name" required>
        </div>
        <div class="col-md-6 col-sm-6">
            <label for="shipping_address" class="form-label"><b>Shipping Address <span
                        class="text-danger">*</span></b></label>
            <input type="text" class="form-control" id="shipping_address" name="shipping_address"
                value="{{ old('shipping_address') }}" placeholder="Shipping Address" required>
        </div>
        <div class="col-md-3 col-sm-6">
            ...
            <select name="area_id" id="area_id" class="select form-select" data-placeholder="Select Area" required>
                ...
                @foreach ($areas as $area)
                    <option value="{{ $area->id }}" data-shipping="{{ $area->shipping_charge }}"
                        {{ old('area_id') && old('area_id') == $area->id ? 'selected' : '' }}>{{ $area->name }}
                    </option>
                @endforeach
            </select>
        </div>
        <div class="col-md-9 col-sm-6">
            ...
            <input type="text" class="form-control" id="order_note" name="order_note"
                value="{{ old('order_note') }}" placeholder="Remarks">
        </div>
        <div class="col-sm-5">
        ...
        <div class="col-md-3 col-sm-6">
            <label for="order_package_id" class="form-label"><b>Package</b></label>
            <select name="order_package_id" id="order_package_id" class="form-select select"
        ...
        <div class="col-12">
            <div class="table-responsive" id="response">
                @include('admin.online_order.partial.table', ['package' => null])
            </div>
        </div>
    </div>
@endsection
...
@push('js')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $(document).on('change', '#order_package_id', function(e) {
                var order_package_id = $(this).val();
                let url = "{{ Route('admin.order.create') }}";
                $.ajax({
                    url: url,
                    type: "POST",
                    data: {
            ...
            $(document).on('change', '#area_id', function(e) {
                let shipping_charge = $('#area_id option:selected').data('shipping');
            ...
            $(document).on('change', '#product_id', function(e) {
                $('#order_package_id').val('');
                if ($('#package_active').length == 1) {
                    $('#response').html(`<table class="table table-bordered table-striped target-table align-middle mb-0">
                            ...
                            if (response.status == 'success') {
                                var tr =
                                    `<tr>
                                        ...
                                        <td><input style="min-width: 80px;" class="form-control input-sm text-center rate" type="number" name="price[${existing_key}]" step="any" value="${response.price}" required></td>
                                        <td><input style="min-width: 80px;" class="form-control input-sm text-center qty" type="number" name="quantity[${existing_key}]" step="any" value="${response.quantity}" required></td>
                                        <td><input style="min-width: 80px;" class="form-control input-sm text-center amount" type="number" name="amount[${existing_key}]" step="any" value="${response.amount}" required></td>
            ...
            $(document).on('click', '.remove_item', function(e) {
                ...
                var shipping_charge = +$('#shipping_charge').val();
                var advance = +$('#advance').val();
                var net_payable = total_amount - discount + shipping_charge - advance;
                $('#total_amount').val(total_amount);
                $('#net_payable').val(net_payable);
            });
            ...
            $('#percentage_input').on('wheel keyup change', function(event) {
                ...
                var advance = +$('#advance').val();
                $("#net_payable").val(total - fix_discount - advance);
            });

            $(document).on('wheel keyup change', '#discount, #shipping_charge, #advance', function(e) {
                var total_amount = +$('#total_amount').val();
                var discount = +$('#discount').val();
                var shipping_charge = +$('#shipping_charge').val();
                var advance = +$('#advance').val();
                $('#net_payable').val(total_amount - discount + shipping_charge - advance);
            });
            ...
            $(document).on('click', '#alt_phone', function(e) {
