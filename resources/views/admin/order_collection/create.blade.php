...
@section('content')
    <div class="row g-3">
        ...
        <div class="col-md-4 col-sm-6">
            <label for="delivery_agent_id" class="form-label"><b>Delivery Agent</b></label>
            <select name="delivery_agent_id" id="delivery_agent_id" class="select form-select"
                data-placeholder="Select Delivery Agent">
                <option value=""></option>
        ...
        <div class="col-md-4 col-sm-6">
            <label for="coa_setup_id" class="form-label"><b>Cash Head</b></label>
            <select name="coa_setup_id" id="coa_setup_id" class="select form-select"
                data-placeholder="Select Cash Head">
                <option value=""></option>
                @foreach ($cash_heads as $item)
                    <option value="{{ $item->id }}" {{ old('coa_setup_id', 154) == $item->id ? 'selected' : '' }}>{{ $item->head_name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="col-md-3 col-sm-6">
            <label for="total_delivery_cost" class="form-label"><b>Total Delivery Cost</b></label>
            <input type="text" id="total_delivery_cost" class="form-control" value="0" readonly>
        </div>
        <div class="col-md-3 col-sm-6">
            <label for="total_amount" class="form-label"><b>Cash Amount</b></label>
                ...
                <table class="table table-bordered table-striped target-table align-middle mb-0">
                    <thead class="bg-primary border-primary text-white align-middle text-nowrap">
                        <tr>
                            ...
                            <th class="py-1">Delivery Agent</th>
                            <th class="py-1">Delivery Cost</th>
                    ...
                    <tbody id="tbody">
                        @foreach ($orders as $item)
                            <tr>
                                ...
                                <td class="text-center">{{ $item->delivery_cost }}</td>
                                <td class="text-center">{{ $item->due }}</td>
                                <td>
                                    <input type="hidden" id="delivery_cost_{{ $item->id }}"
...
@push('js')
    <script type="text/javascript">
        $(document).ready(function() {
                    ...
                    success: (response) => {
                        if (response.status == 'success') {
                            ...
                            $.each(response.orders, function(key, item) {
                                var tr =
                                    `<tr>
                                        <td class="text-center" class="text-center">${++key}</td>
                                        <td class="text-nowrap">${item.formattedDate}</td>
                                        <td>${item.invoice}</td>
                                        <td>${item.user_name}</td>
                                        <td>${item.user_phone}</td>
                                        ...
                                        <td class="text-center">${item.due}</td>
                                        <td>
                                            <input type="hidden" id="delivery_cost_${item.id}" value="${item.delivery_cost}">
                                            <input type="hidden" id="due_${item.id}" value="${item.due}">
                                            <div class="custom-control custom-checkbox w-fit mx-auto">
                                                <input type="checkbox" class="custom-control-input order_id" name="order_id[]"
                                                    value="${item.id}" id="order_id${item.id}">
            ...
            $(document).on('change', '#delivery_agent_id', function(e) {
                var delivery_agent_id = $(this).val();
            ...
            function calc() {
                ...
                $('.order_id:checked').each(function(index, value) {
                    ...
                    total_delivery_cost += +$('#delivery_cost_' + order_id).val();
                });
                $('#total_amount').val(total_amount.toFixed(2) - total_delivery_cost.toFixed(2));
                $('#total_delivery_cost').val(total_delivery_cost.toFixed(2));
