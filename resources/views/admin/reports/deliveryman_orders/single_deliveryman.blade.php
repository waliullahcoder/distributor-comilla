            ...
            <form action="{{ @$filter_link }}" method="GET" class="filter_form">
                ...
                <div class="d-flex align-items-center justify-content-between gap-2">
                    ...
                    <div class="flex-shrink-0">
                        <a href="{{ Route('admin.deliveryman-orders.index') }}" class="btn btn-primary btn-sm">Go Back</a>
                        <input type="hidden" name="view_orders" value="true">
                        <input type="hidden" name="delivery_agent_id" value="{{ request('delivery_agent_id') }}">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table id="dataTable" class="table table-bordered table-sm">
                    ...
                    <tbody>
                        @foreach ($data as $row)
                            <tr>
                                ...
                                <td class="px-3 text-center">{{ number_format($row->due - $row->advance) }}</td>
                                <td class="px-3">
                                    @php
                                        $bg = 'bg-primary';
                    ...
                    <tfoot>
                        <th colspan="7" class="px-3 text-end">Total</th>
                        <th class="px-3 text-center">{{ $data->sum('due') - $data->sum('advance') }}</th>
                        <th class="px-3"></th>
                    </tfoot>
                </table>
            </div>
