        ...
        <table id="dataTable" class="table table-bordered table-sm">
            <thead class="text-nowrap">
                <tr>
                    ...
                    <th class="px-3 text-center">Route Cancel</th>
            ...
            <tbody>
                ...
                @for ($i = 1; $i <= 31; $i++)
                    <tr>
                        ...
                        <th class="px-3 text-nowrap">Date - {{ $i }}</td>
                        ...
                        <td class="px-3 text-center">
                            @php
                                if ($date == $year . '-' . $month . '-' . (Str::length($i) == 1 ? '0' . $i : $i)) {
                                    ...
                                    $total_cancelled = $query->whereNull('delivery_man_id')->count();
                        ...
                        <td class="px-3 text-center">
                            @php
                                if ($date == $year . '-' . $month . '-' . (Str::length($i) == 1 ? '0' . $i : $i)) {
                                    ...
                                    $total_cancelled = $query->whereNotNull('delivery_man_id')->count();

                                    $total_8 += $total_cancelled;
                                    if ($total_cancelled > 0) {
                                        echo '<a class="fs-15" href="' .
                                            Route('admin.monthly-statement.index') .
                                            '?view_orders=true&date=' .
                        ...
                        <th class="px-3 text-center">
                            @php
                                if ($date == $year . '-' . $month . '-' . (Str::length($i) == 1 ? '0' . $i : $i)) {
                                    $query = \App\Models\OnlineSales::where('date', $date)->where('collected', 1);
                                    if (request('store_id')) {
                                        $query->where('store_id', request('store_id'));
                                    }
                                    $total_business = $query->sum('amount');
                                    $order_ids = $query->groupBy('order_id')->pluck('order_id')->toArray();
                                    $total_business += \App\Models\Order::whereIn('id', $order_ids)->sum(
                                        'shipping_charge',
                                    );
            ...
            <tfoot>
                <tr>
                    ...
                    <th class="px-3 text-center">{{ number_format($total_8) }}</th>
