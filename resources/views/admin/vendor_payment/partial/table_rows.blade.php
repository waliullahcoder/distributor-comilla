@foreach ($purchases as $item)
    ...
    <tr>
        ...
        <td>
            <input type="number" class="form-control input-sm py-1 text-end due" step="any" id="due_{{ $item->id }}"
                name="due[{{ $item->id }}]" value="{{ $due }}" readonly>
        </td>
        <td>
            <input type="number" id="amount_{{ $item->id }}" name="curr_payment[{{ $item->id }}]"
                class="form-control input-sm py-1 text-end amount" max="{{ $due + @$old_data->paid }}" data-id="{{ $item->id }}"
                step="any" value="{{ $paid }}">
        </td>
