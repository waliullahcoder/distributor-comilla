@foreach ($purchases as $item)
    <tr>
        ...
        <td>
            <input type="number" class="form-control py-1 input-sm text-end due" step="any" id="due_{{ $item->id }}"
        ...
        <td>
            <input type="number" id="amount_{{ $item->id }}" name="curr_payment[{{ $item->id }}]"
                class="form-control py-1 input-sm text-end amount" max="{{ $item->net_amount - $item->return_amount - $item->total_paid }}"
