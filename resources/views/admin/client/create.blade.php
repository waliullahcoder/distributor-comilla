...
@section('content')
    <div class="row g-3">
            ...
            <select name="reference_by" id="reference_by" class="select form-select" data-placeholder="Select Reference">
                ...
                @foreach ($staffs as $staff)
                    <option value="{{ $staff->id }}"
                        {{ old('reference_by') && old('reference_by') == $staff->id ? 'selected' : '' }}>{{ $staff->name }}
                    </option>
                @endforeach
            </select>
        </div>
            ...
            <select name="area_id" id="area_id" class="select form-select" data-placeholder="Select Area" required>
                ...
                @foreach ($areas as $area)
                    <option value="{{ $area->id }}"
                        {{ old('area_id') && old('area_id') == $area->id ? 'selected' : '' }}>
                        {{ $area->name }}
                    </option>
                @endforeach
            </select>
            ...
            <select name="client_category_id" id="client_category_id" class="select form-select"
                ...
                @foreach ($categories as $category)
                    <option value="{{ $category->id }}"
                        {{ old('client_category_id') && old('client_category_id') == $category->id ? 'selected' : '' }}>
                        {{ $category->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="col-sm-6">
            <label for="code" class="form-label"><b>Client Code</b></label>
        ...
        <div class="col-md-3 col-sm-6">
            <div class="row g-3 pt-2">
                <div class="col-6">
                    <label class="form-label text-white"> VAT Applicable</label>
        ...
        <div class="col-sm-6" id="chain_client" style="display: none;">
            ...
            <select name="chain_client_id" id="chain_client_id" class="select form-select"
                ...
                <option value="NULL">Select Chain Client</option>
                @foreach ($chain_clients as $chain_client)
                    <option value="{{ $chain_client->id }}"
                        {{ old('chain_client_id') && old('chain_client_id') == $chain_client->id ? 'selected' : '' }}>
                        {{ $chain_client->name }}
                    </option>
                @endforeach
            </select>
